            ...
            <tr key={i} className="bg-slate-950/40 hover:bg-slate-900/40">
              {row.map((cell, j) => (
                <td key={j} className={`px-3 py-1.5 whitespace-nowrap font-mono ${
                  !cell || cell === 'NULL' ? 'text-red-400/70 italic' : 'text-slate-300'
                }`}>
                  {cell || <span className="text-red-400/60 text-[10px]">空（NULL?）</span>}
                </td>
              ))}
            </tr>
          ))}
...
export function SaasSourceStage({ quest, dbReady, onComplete }: Props) {
    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-2xl mx-auto p-6 space-y-5">
        ...
        <div className="px-4 py-3 rounded-xl bg-blue-500/10 border border-blue-500/20">
          <p className="text-xs text-blue-400 font-medium uppercase tracking-wider mb-1">このステージで学ぶこと</p>
          <p className="text-white text-sm font-medium">SaaSの3データソース：課金・顧客属性・利用ログを生のまま格納</p>
        </div>

        {/* CSV tabs */}
        <div>
          <p className="text-slate-400 text-sm mb-3">CloudStackから受け取ったデータ：</p>
          ...
          {quest.csvFiles[activeFile].name === 'subscriptions' && (
            <p className="text-amber-400 text-xs mt-2">⚠️ cancelled_at 列に空のセルがあります。NULL なのか空文字なのか？これが次のステージの課題です。</p>
          )}
        </div>

        {/* Rules */}
        <div className="px-4 py-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20">
          <p className="text-xs text-indigo-400 font-medium uppercase tracking-wider mb-3">Source Layer のルール（SaaS版）</p>
          <ul className="space-y-2">
            {[
              ['課金データ（Stripe）は加工しない', '解約日時・MRRは原本を保持'],
              ['イベントログも生のまま格納', 'ユーザー行動の「事実」を残す'],
